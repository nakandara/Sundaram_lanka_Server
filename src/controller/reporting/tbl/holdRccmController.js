...
exports.getHoldRccm= async (req, res) => {
    try {
        const results = await db.query(`select * from std.hold_rccm ORDER BY in_date DESC   `)
        res.status(200).json({ 
            status: "succes",
            data: {
                data: results.rows,
...
exports.updateHoldRccm = async (req, res) => {
  ...
  try {
    ...
    const resu = await db.query(
      "UPDATE std.hold_rccm SET ap_no = $2,accountability=$3,metix=$4,target_date=$5,actual=$6,reason_achive_not_target=$7,root_cause=$8,counter_measure=$9,resp=$10,target=$11,status=$12 where   id = $1 ",
