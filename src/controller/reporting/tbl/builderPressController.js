...
exports.getBuilder = async (req, res) => {
    try {
        const results = await db.query(`select sn,mfgdate,actwgt from mfg.builder where mfgdate >= '${req.params.date} 7:00:00 ' and mfgdate <='${req.params.datetwo} 7:00:00 '  ORDER BY sn asc`)
        res.status(200).json({ 
            status: "succes",
            data: {
                data: results.rows,
...
exports.getPress = async (req, res) => {
    try {
        const results = await db.query(`select * from std.press_new_data where date >= '${req.params.datethree}  07:00:00.000000' and date <='${req.params.datefour} 07:00:00.000000'  ORDER BY sn desc`)
        res.status(200).json({ 
            status: "succes",
            data: {
                data: results.rows,
    ...
    //select * from restaurants left join (select restaurant_id, COUNT(*), TRUNC(AVG(rating),1) as average_rating from reviews group by restaurant_id) reviews on restaurants.id = reviews.restaurant_id where id = $1
      try {
        const results = await db.query(
          "select sn,mfgdate AT TIME ZONE 'UTC' ,actwgt from mfg.builder WHERE sn=$1",
          [req.params.sn]
        );
